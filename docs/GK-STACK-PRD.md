...
Curated set: `gk-init` (legacy onboarding — generates grounding docs from observed code only), `gk-graph` (dependency graph with confidence tags + god nodes; feeds context slicing), `gk-parallel` (wave-based subagent execution with conflict detection), `gk-fix-bug`, `gk-test` (blast-radius-aware), `gk-perf`, `gk-doctor`, `gk-migrate` (gated migration campaigns, implements R16). Optional per team taste: `gk-brainstorm`, `gk-architect`, `gk-document`, `gk-changelog`, `gk-release`, `gk-incident`.
Acceptance criteria:
...
- **R16 — Large-scale migration campaign skill (`gk-migrate`):** codifies the plan → 3-file pilot → headless batch (`claude -p` per file, Sonnet/Haiku, scoped tools) → verify → rollup-PR pattern for repo-wide codemods across thousands of files. *Design note: pilot results must gate the batch; per-file failures logged, never retried more than twice.* **Status: skill shipped in `skills-library/gk-migrate/` (v1.2); the P2 item that remains is running and hardening a real campaign with it.**
