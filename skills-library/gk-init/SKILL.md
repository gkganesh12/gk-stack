...
name: gk-init
description: >-
  Onboards an existing or legacy codebase into the gk-* skills framework by
  reading actual source files and generating the CLAUDE.md and full gk/
  folder that all gk-* skills need. Use whenever the user has an existing
  codebase with no gk/ folder and wants any gk-* skill — even without the
  exact trigger phrases. Triggers on "gk-init", "onboard this project",
  "init the gk folder", "set up gk for this project", "this is a legacy
  project", "retrofit this codebase", "I want to use gk skills on an
  existing project", "add gk to this repo", "the gk folder doesn't
  exist", "can we use gk skills here".
---

# GK Init Skill

Reverse-engineers the gk-* framework scaffold from an existing codebase.
Reads actual source files. Infers everything from code. Documents only what is observed.
Produces a complete gk/ folder that immediately unblocks all gk-* skills.
...
Every decision made through gk-* skills from this point forward gets logged.
...
It is marked clearly. gk-review will flag it until a human verifies it.
...
Check if gk/ already exists:
```bash
ls -d gk/ 2>/dev/null && echo "EXISTS" || echo "NOT FOUND"
```

If gk/ already exists:
> "A gk/ folder already exists in this project. Running gk-init will overwrite it.
...
```
> 📥 Generated by gk-init on [date] from codebase analysis.
...
This is the most important file for unblocking gk-add-feature and gk-fix-bug.
...
```
> ⚠️ PROVISIONAL SPEC — generated by gk-init from observed code behaviour.
> This describes what the code DOES, not necessarily what it was INTENDED to do.
> Verify this spec before using gk-review — incorrect specs will pass broken code.
...
Generate compressed map of SPEC.md. Follow same format as gk-new-app.
...
> Onboarded via gk-init on [date]. All decisions prior to this date are untracked.
...
### D-001 — Project onboarded via gk-init
- **Date**: [today] · **Task**: gk-init · **Type**: discovery
- **What was planned**: N/A — legacy project, no prior gk context
- **What was done**: Generated gk/ scaffold from codebase analysis.
  Stack: [confirmed stack]. Pattern: [confirmed pattern].
  Files read: [N] files across [N] directories.
- **Why**: Retrofitting gk-* skills framework onto existing codebase.
- **Alternatives considered**: Manual documentation (rejected — too slow and error-prone)
- **Impact on other tasks**: All gk-* skills now operational.
  SPEC.md is provisional — verify before first gk-review run.
- **Approved by**: human (triggered gk-init)
```
...
> This project was not built using the gk-* framework.
...
## Phase gates
(not applicable — project pre-dates gk-* framework)
...
Create `gk/reviews/backlog.md`:
```markdown
# Review backlog
> Started on [date] via gk-init.
...
- Session startup reads `gk/CODEBASE.md` first (existing code, not scaffolded)
- Phase gates section replaced with: "No phase structure — this is an existing project. Use gk-review on demand."
- Active feature section still present — used by add-feature and fix-bug normally
- Note at top: "Onboarded via gk-init on [date]"
...
```
✅ gk-init complete — [Project Name]

GENERATED FILES
  CLAUDE.md                   ← project root
  gk/ARCHITECTURE.md        ← [N] patterns documented
  gk/CODEBASE.md            ← [N] files mapped, [N] routes, [N] models
  gk/SPEC.md                ← PROVISIONAL — [N] features inferred
  gk/SPEC_INDEX.md
  gk/DECISIONS.md           ← D-001 bootstrap entry
  gk/TASKS.md
  gk/reviews/backlog.md
...
⚠️  BEFORE RUNNING gk-review:
  Scan gk/SPEC.md and verify the inferred features are correct.
...
WHAT WORKS NOW
  ✅ gk-add-feature  — CODEBASE.md and ARCHITECTURE.md ready
  ✅ gk-fix-bug      — CODEBASE.md and ARCHITECTURE.md ready
  ✅ gk-review       — works, but note PROVISIONAL SPEC warning above
  ✅ gk-design       — CODEBASE.md ready; DESIGN_SYSTEM.md generated on first design: run
  ✅ gk-progress     — TASKS.md ready (starts from today)
  ✅ gk-change-spec  — SPEC.md ready (verify first)
  ✅ gk-graph        — dependency graph built from CODEBASE.md
...
After outputting the completion report, invoke `gk-graph: build`.
...
The graph enables gk-fix-bug, gk-test, and gk-review to query
relationships directly instead of loading the full CODEBASE.md.

```
gk-graph: build complete
  Files indexed from CODEBASE.md: [N]
  Concepts mapped: [N]
  Graph: gk/graph/ (3 files generated)
  Next update: automatic after next gk-add-feature or gk-fix-bug
```
...
don't apply to existing code. Its absence will not break any gk-* skill.
