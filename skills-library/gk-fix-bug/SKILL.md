...
**Human-facing:**
- `gk/TASKS.md` — plain English, updated after every task

**Agent-facing (human never opens these):**
- `gk/bugs/[date-slug]/BUG_SPEC.md` · `BUG_PLAN.md` · `BUG_TASKS.md`
- `gk/CODEBASE.md` · `gk/ARCHITECTURE.md` · `gk/DECISIONS.md`
...
**Environment bug** — route to gk-doctor, NOT the bug workflow:
...
Invoke `gk-doctor`. After doctor: completes:
...
## Step 2 — Read gk/CODEBASE.md (significant bugs only)
...
Also read gk/ARCHITECTURE.md — the fix must follow existing patterns.
Also read gk/DESIGN_SYSTEM.md if the bug is in a styled component.
...
```bash
ls gk/graph/DEPENDENCY_GRAPH.json 2>/dev/null && echo "GRAPH EXISTS" || echo "NO GRAPH"
```

**If graph exists — query it for the suspected file:**
```
gk-graph: query [suspected file from Step 2]
```
...
```
gk-graph: query [concept name from graph result]
```
...
Wait for confirmation. Save as `gk/bugs/[date-slug]/BUG_SPEC.md`.
...
Wait for approval. Save as `gk/bugs/[date-slug]/BUG_PLAN.md`.
...
> Folder: gk/bugs/[date-slug]/ | Added: [date]

**Files in scope**: [only files from BUG_PLAN.md]
**Files out of scope**: [everything else]
**Design system**: [check gk/DESIGN_SYSTEM.md if bug is in styled component]
**Scope changes**: If user says "change:" — stop and run gk-change-spec.
...
Save as `gk/bugs/[date-slug]/BUG_TASKS.md`.

---

## Step 8 — Update gk/TASKS.md
...
🐛 [Bug summary in plain English] (0/4)
   ...
   [ ] BUG-004 · Update docs
   → Full specs: gk/bugs/[date-slug]/BUG_TASKS.md (agent use)
```
...
## Step 9 — Update gk/DECISIONS.md

```
---
### D-[ID] — Bug fix: [Bug summary]
- **Date**: [date] · **Type**: drift
- **Folder**: gk/bugs/[date-slug]/
...
> ✅ **Bug fix kit ready.**
>
> **Created:** gk/bugs/[date-slug]/
>   BUG_SPEC.md · BUG_PLAN.md · BUG_TASKS.md
> **Updated:** gk/TASKS.md · CLAUDE.md · gk/DECISIONS.md
...
> gk/TASKS.md. Confirm first task before writing any code.
> ```
...
After the fix is complete and verified, invoke `gk-graph: update`.
