...
name: gk-test
description: >-
  Blast-radius-aware test generation for gk-* projects — traces everything
  ...
  the end of gk-add-feature and gk-fix-bug sessions.
---
...
### Entry point B — After gk-add-feature
...
### Entry point C — After gk-fix-bug
...
1. `gk/ARCHITECTURE.md` — testing philosophy, framework, conventions, test location rules
2. `gk/CODEBASE.md` — section 1 (stack), section 2 (commands), section 4 (key files),
   section 5 (routes), section 7 (components), section 10 (patterns)
3. `gk/SPEC.md` — acceptance criteria (drives what behaviour must be verified)
4. If Entry B: `gk/features/[date-slug]/FEATURE_SPEC.md` + `FEATURE_TASKS.md`
5. If Entry C: `gk/bugs/[date-slug]/BUG_SPEC.md` + `BUG_TASKS.md`
...
- Read `gk/CODEBASE.md` to locate relevant files
...
Existing regression tests:
  [test file] — [what it covers] (Entry C only — from gk-fix-bug)
```
...
Read `gk/ARCHITECTURE.md` for the component testing approach.
Check `gk/DESIGN_SYSTEM.md` if it exists — visual states may need testing.
