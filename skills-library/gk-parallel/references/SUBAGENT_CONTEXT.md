...
Read during Step 3 of gk-parallel.
How to build targeted context slices for each subagent.
Graph-aware slicing when gk-graph is available.
...
```
GRAPH_AVAILABLE:    gk/graph/DEPENDENCY_GRAPH.json exists
...
concept_graph = json.loads(Path("gk/graph/CONCEPT_GRAPH.json").read_text())
meta = json.loads(Path("gk/graph/.graph-meta.json").read_text()) \
       if Path("gk/graph/.graph-meta.json").exists() else {}
...
def render_context_block(slice_data, task):
    ...
    if slice_data["arch_patterns"]:
        ...
        lines.append(f"  (Full rules in gk/ARCHITECTURE.md — read the relevant section)")
    ...
    lines.append("\nDo NOT load gk/CODEBASE.md — your context above is sufficient.")
    lines.append("Read gk/ARCHITECTURE.md if you need pattern details.")
...
Read the following files for project context:
  gk/CODEBASE.md         ← file map, routes, models, agents
  gk/ARCHITECTURE.md     ← patterns and conventions to follow

Your task files: [Touches field]

Note: gk-graph is not installed. Install it to reduce per-subagent
