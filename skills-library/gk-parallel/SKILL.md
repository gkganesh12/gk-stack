...
name: gk-parallel
description: >-
  Executes independent tasks from gk-* task files as parallel Claude Code
  ...
  gk-add-feature, gk-fix-bug, gk-new-app when GK_MODE=autonomous
  or parallel tasks exist and user approves.
---

# GK Parallel Skill v2
...
- Graph-aware context slicing per subagent (if gk-graph installed)
- Structured subagent reporting with [x] / [~] / [!] states
- Diagnostic retry on failure (targeted, not blind)
- Live wave progress log written to gk/parallel/
- Post-wave gk-graph update
- Wave cost annotation for gk-cost
...
## How it integrates with GK_MODE
...
Always shows the plan and asks — regardless of GK_MODE.
...
Read `gk/TASKS.md` for all pending tasks.
Always show plan + ask (y/n), regardless of GK_MODE.

### Entry point B — Called by gk-add-feature
Source: `gk/features/[date-slug]/FEATURE_TASKS.md`
GK_MODE=autonomous → skip prompt, launch directly.
GK_MODE=manual → show plan, ask (y/n).

### Entry point C — Called by gk-fix-bug
Source: `gk/bugs/[date-slug]/BUG_TASKS.md`
Same mode logic as Entry B.

### Entry point D — Called by gk-new-app
Source: `gk/TASKS.md` Phase 1 tasks only.
Same mode logic as Entry B.
...
Read `gk/ARCHITECTURE.md` — conventions every subagent must follow.

**Check for gk-graph:**
```bash
ls gk/graph/DEPENDENCY_GRAPH.json 2>/dev/null && echo "GRAPH_AVAILABLE" || echo "NO_GRAPH"
ls gk/graph/.graph-meta.json 2>/dev/null && echo "META_AVAILABLE" || echo "NO_META"
```
...
Exception: `gk/CODEBASE.md` and `gk/DECISIONS.md` — append-only, safe.
...
CODEBASE.md is NOT loaded by subagents when gk-graph is available.
...
Context mode: [graph-aware slicing (gk-graph) | full CODEBASE.md]
...
```bash
mkdir -p gk/parallel
cat > gk/parallel/wave-[N]-status.md << 'EOF'
...
[IF no graph:]
  Read gk/CODEBASE.md and gk/ARCHITECTURE.md for project context.
...
The main session updates `gk/CODEBASE.md` once — not during parallel execution.
...
**Update gk/TASKS.md "What just happened":**
```
...
## Step 9 — gk-graph update (post-wave)
...
If gk-graph not installed — skip this step silently.
...
Totals:
  Tasks complete: [N]/[N]
  Tasks partial:  [N] (see gk/parallel/ for unmet criteria)
  Time taken:     [Xm]
  Est. sequential: ~[N]h
...
Progress log: gk/parallel/
```

**In autonomous mode:**
Signal completion to the calling skill.
Do not stop — let gk-add-feature / gk-fix-bug / gk-new-app
...
`gk-graph: update` runs after wave completion.
...
Even if gk-cost is not installed — the annotation is written
to the progress log so gk-cost can read it later.
