...
Read during gk-graph: init before building the spec graph.
...
- gk-graph: update runs after file creation and git diff confirms the relationship
...
They get populated by `gk-graph: update` as files are created and git diff
...
When `gk-graph: update` runs after a file is created:
