...
Read during gk-graph: build (existing projects via gk-init).
...
AMBIGUOUS  — agent couldn't determine clearly, flagged for human review
             confidence: < 0.6
             surface in gk-graph: status as unresolved
```
...
"src/agents/scout_agent.py": {
  ...
  "concept_evidence": "gk/features/2026-04-01-competitor-discovery/FEATURE_TASKS.md Touches field",
  "imports": [
    {
...
- gk-fix-bug: reads rationale before diagnosing — knows *why* the file works the way it does before proposing a fix
- gk-review: detects intent drift — code that contradicts documented rationale is a P1 finding
- gk-document: generates accurate WHY documentation rather than inferring from code alone
- gk-handoff: surfaces design decisions automatically in CONTEXT_DUMP.md and ONBOARDING.md
...
Rationale gets populated on first `gk-graph: update` after files are read.
...
1. **EXTRACTED (confidence 1.0):** gk/features/ folder — file listed in FEATURE_TASKS.md Touches field
...
def assign_concept(file_path, codebase_md, feature_folders, concept_graph):
    ...
    for folder in feature_folders:
        tasks_file = f"gk/features/{folder}/FEATURE_TASKS.md"
...
**God nodes are surfaced in gk-graph: status** and passed to:
- gk-review: read god nodes first, highest coupling = highest review priority
- gk-fix-bug: if suspected file IS a god node, flag large blast radius before diagnosis
- gk-parallel: god node files are never written by two waves simultaneously
- gk-cost: god nodes loaded in every session = CP-01 pattern candidate
...
Surface AMBIGUOUS items in `gk-graph: status` under "Needs review".
