...
Read by gk-fix-bug, gk-test, and gk-review at task start.
How to query the graph efficiently, filter by confidence,
read rationale nodes, and use god node data.

---

## Query pattern 1 — gk-fix-bug (blast radius with confidence tiers)

```python
import json
from pathlib import Path

def get_blast_radius(suspect_file, graph_path="gk/graph/DEPENDENCY_GRAPH.json"):
...
```python
def get_rationale(file_path, graph_path="gk/graph/DEPENDENCY_GRAPH.json"):
    graph = json.loads(Path(graph_path).read_text())
...
```python
def check_if_god_node(file_path, meta_path="gk/graph/.graph-meta.json"):
    try:
...
## Query pattern 2 — gk-test (confidence-tiered blast radius)

```python
def get_test_scope(changed_files, graph_path="gk/graph/DEPENDENCY_GRAPH.json"):
    graph = json.loads(Path(graph_path).read_text())
...
## Query pattern 3 — gk-review (god nodes first, then concept boundaries)

```python
def review_pre_screening(graph_path="gk/graph/DEPENDENCY_GRAPH.json",
                          meta_path="gk/graph/.graph-meta.json"):
...
## Query pattern 4 — gk-parallel (conflict detection with god nodes)
...
After the session, `gk-graph: update` will formalise the new node.
