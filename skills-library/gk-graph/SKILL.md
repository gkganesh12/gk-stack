...
description: >-
  ...
  concept-level) for gk-* projects so skills can query blast radius and
  rationale instead of loading full CODEBASE.md. Triggers on
  "gk-graph: build", "gk-graph: update", "gk-graph: init",
  "gk-graph: rebuild", "gk-graph: query", "gk-graph: status";
  queried automatically by gk-fix-bug, gk-test, gk-review,
  gk-parallel.
---

# GK Graph Skill

Dual-layer dependency graph for gk-* projects.
...
```
gk-graph: init      ← called by gk-new-app after scaffold created
gk-graph: build     ← called by gk-init after CODEBASE.md written
gk-graph: update    ← called by gk-add-feature and gk-fix-bug at session end
gk-graph: rebuild   ← full rebuild from scratch (recovery command)
gk-graph: query [file or concept]  ← manual query during debugging
gk-graph: status    ← graph health, coverage, god nodes, ambiguous edges
```
...
git status 2>/dev/null || { echo "Not a git repo — gk-graph requires git"; exit 1; }

# Create graph directory
mkdir -p gk/graph

# Check for existing graph
ls gk/graph/DEPENDENCY_GRAPH.json 2>/dev/null && echo "GRAPH EXISTS" || echo "NEW GRAPH"
```

---

## Command: gk-graph init

**Called by:** `gk-new-app` after scaffold is created.
...
```bash
cat gk/SPEC.md
cat gk/ARCHITECTURE.md
cat gk/PLAN.md 2>/dev/null
...
Write `gk/graph/DEPENDENCY_GRAPH.json` — all planned file nodes.
Write `gk/graph/CONCEPT_GRAPH.json` — all planned concept nodes.
Write `gk/graph/.graph-meta.json`:
```json
{
  "version": 2,
...
**Step 7 — Tell gk-new-app:**
```
gk-graph: init complete
  Planned concepts: [N]
  Planned files:    [N]
  INFERRED edges:   [N] (all edges — code not yet written)
  Graph location:   gk/graph/
  State: SPEC GRAPH — edges transition to EXTRACTED as files are built
```

---

## Command: gk-graph build

**Called by:** `gk-init` after CODEBASE.md is written.
...
graph = json.loads(Path('gk/graph/DEPENDENCY_GRAPH.json').read_text())
...
gk-graph: build complete
  ...
  EXTRACTED edges:  [N]
  INFERRED edges:   [N]
  AMBIGUOUS edges:  [N] (see gk-graph: status for details)
...
## Command: gk-graph update

**Called by:** `gk-add-feature` and `gk-fix-bug` at session end.
...
gk-graph: update complete
  ...
  AMBIGUOUS edges:           [N] (run gk-graph: status to review)
...
## Command: gk-graph query

**Called by:** `gk-fix-bug`, `gk-test`, `gk-review` at task start.
Also available manually for debugging.

Read `references/GRAPH_QUERY_PATTERNS.md` for the full query protocol.

```
gk-graph: query scout_agent.py
gk-graph: query competitor-discovery
gk-graph: query POST /api/brand-dna
```
...
Graph health: ⚠️ 3 AMBIGUOUS edges need review
  Run: gk-graph: status to see full details
  Resolve: manually update CODEBASE.md and run gk-graph: update

Files: gk/graph/DEPENDENCY_GRAPH.json (47 nodes, 142 edges)
       gk/graph/CONCEPT_GRAPH.json    (8 concepts)
       gk/graph/graph.html            (interactive — solid=EXTRACTED, dashed=INFERRED)
       gk/graph/.graph-meta.json      (god nodes, ambiguous count, meta)
```

---

## How gk-fix-bug uses the graph
...
## How gk-test uses the graph

At blast radius detection start:

```
1. Get changed files from git diff (already done by gk-graph: update)
...
## How gk-parallel uses the graph
...
`gk-graph: status` surfaces them every time.
