...
name: gk-perf
description: >-
  Full-stack performance audit for gk-* projects — measures and scores
  ...
  "cold start", "perf regression"; runs automatically after gk-review
  flags performance as P1/P2 and as a mandatory gate before gk-deploy.
---
...
### Entry point B — After gk-review flags perf
Triggered when `gk-review` produces P1/P2 findings tagged as performance.

Read the review report. Scope to flagged files and layers only.
Go to **Step 0**.

### Entry point C — Pre-deploy gate
Triggered automatically before `gk-deploy` runs.
...
1. `gk/ARCHITECTURE.md` — stack, patterns, any perf thresholds defined
2. `gk/CODEBASE.md` — section 1 (stack), section 2 (commands),
   section 5 (routes), section 7 (components), section 8 (services)
3. `CLAUDE.md` — dev/build/test/run commands
4. `gk/perf/baseline.json` — previous run data (if exists)
...
```bash
mkdir -p gk/perf
ls gk/perf/baseline.json 2>/dev/null && echo "BASELINE EXISTS" || echo "FIRST RUN"
```
...
lhci autorun \
  ...
  --upload.outputDir=gk/perf/lighthouse
```
...
npx vite-bundle-visualizer --json 2>/dev/null \
  > gk/perf/bundle-analysis.json

# Next.js
ANALYZE=true npm run build 2>&1 | tee gk/perf/bundle-build.log
...
3. Add inline comment: `# perf: [what fixed] — gk-perf [date]`
...
python3 -c "
import json, os
path = 'gk/perf/history.json'
history = json.load(open(path)) if os.path.exists(path) else []
history.append(json.load(open('gk/perf/baseline.json')))
...
**gk/TASKS.md** — update "What just happened":
```
✅ Performance audit — [date] — Overall [N]/100 [↑/↓/→ vs last run]
   [N] fixes written · [N] pending manual review
   Report: gk/perf/report-[date].md
```

**gk/reviews/backlog.md** — add unfixed P1/P2 findings as perf backlog items.
Resolve any previously logged perf items that were fixed in this run.

**gk/DECISIONS.md** — log any Category B fix that changed an API shape
...
Save to `gk/perf/report-[YYYY-MM-DD].md`.

```
✅ gk-perf complete — [Project Name] — [date]
...
Full report: gk/perf/report-[date].md
Baseline:    gk/perf/baseline.json (run #[N])
History:     gk/perf/history.json
```
