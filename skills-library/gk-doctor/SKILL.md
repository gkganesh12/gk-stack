...
name: gk-doctor
description: >-
  Environment health check with safe auto-remediation for gk-* projects —
  ...
  start"; also invoked by gk-fix-bug when an environment bug is detected
  in triage.
---
...
The most common session-wasting pattern in gk-* projects:
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "[ -f gk/.doctor-last-run ] && [ $(( $(date +%s) - $(cat gk/.doctor-last-run 2>/dev/null || echo 0) )) -lt 3600 ] || echo 'DOCTOR: Run doctor: to check environment health'"
      }]
    }]
  }
}
...
For full auto-run on every session, gk-mode: autonomous enables this automatically.
...
ls gk/ 2>/dev/null && echo "GK=true" || echo "GK=false"
```
...
### Check 10 — gk/ folder integrity (if gk project)

```bash
if [ -d "gk" ]; then
  missing=""
  [ ! -f "gk/TASKS.md" ] && missing="$missing TASKS.md"
  [ ! -f "gk/SPEC.md" ] && missing="$missing SPEC.md"
  [ ! -f "CLAUDE.md" ] && missing="$missing CLAUDE.md(root)"
  
  if [ -n "$missing" ]; then
    echo "WARN:gk/ exists but missing:$missing"
  else
    echo "PASS:gk/ folder complete"
  fi
else
  echo "INFO:not a gk project (no gk/ folder)"
fi
```
...
✅ gk/ folder      — TASKS.md, SPEC.md, CLAUDE.md present
...
```bash
mkdir -p gk
date +%s > gk/.doctor-last-run
```

This allows gk-progress to show env health status in the dashboard
...
## Called by gk-fix-bug

When gk-fix-bug detects an environment bug in triage (Step 1),
it calls gk-doctor automatically:

```
gk-fix-bug detected: environment bug
Running doctor: to diagnose before bug workflow...
[doctor output]
If environment is fixed and app starts → close bug, no BUG_SPEC needed.
If code bug remains → continue with gk-fix-bug significant path.
```
