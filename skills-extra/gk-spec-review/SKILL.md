...
description: >-
  ...
  invoked automatically by gk-brainstorm, gk-agent, gk-new-app,
  gk-init, and gk-add-feature after they write planning documents.
---

# GK Spec Review Skill
...
### Trigger 1 — After `gk-brainstorm`
Invoked by: the final step of `gk-brainstorm` after BRIEF.md is written.
...
### Trigger 2 — After `gk-agent`
Invoked by: the final step of `gk-agent` after AGENT_ARCH.md is written.
...
### Trigger 3 — After `gk-new-app` or `gk-init`
Invoked by: the final step of `gk-new-app` / `gk-init` after gk/ folder is created.
Announcement: *"gk/ folder created. Running spec-review before first task..."*
...
### Trigger 4 — After `gk-add-feature`
Invoked by: the final step of `gk-add-feature` after FEATURE_SPEC.md is written.
...
ls BRIEF.md AGENT_ARCH.md SPEC.md ARCHITECTURE.md \
   gk/SPEC.md gk/ARCHITECTURE.md \
   gk/features/*/FEATURE_SPEC.md 2>/dev/null
```
...
## Step 3 — Audit SPEC.md / gk/SPEC.md
...
If SPEC.md contains `⚠️ PROVISIONAL` (set by gk-init):
...
## Step 4 — Audit ARCHITECTURE.md / gk/ARCHITECTURE.md
...
[If P0 or P1 found:]
  ...
  with findings logged to gk/spec-reviews/[date].md.
...
Log all findings to `gk/spec-reviews/[YYYY-MM-DD]-[trigger].md`.
Update `gk/TASKS.md` with a note:
```
⚠️ Spec review findings logged — [N] P0, [N] P1 unresolved
   See: gk/spec-reviews/[date]-[trigger].md
```
...
```
[spec-review: N P0, N P1 found — logged to gk/spec-reviews/]
```
