...
name: gk-review
description: >-
  Evidence-based code review for gk-* projects — the mandatory quality
...
1. `gk/ARCHITECTURE.md` — primary reference for drift detection
2. `gk/CODEBASE.md` — exact file paths as-built
3. `gk/SPEC.md` — acceptance criteria to verify against
4. `gk/DECISIONS.md` — past decisions, context for why things are as they are
5. `gk/PLAN.md` — phase scope, what was meant to be built
6. `gk/TASKS.md` — what tasks completed this phase
7. `gk/reviews/` — all previous review files for carryover tracking
8. `BRIEF.md` *(if exists)* — original intent and core value
9. `CLAUDE.md` — boundaries and conventions

**If gk/ARCHITECTURE.md does not exist:**
...
```bash
ls gk/graph/CONCEPT_GRAPH.json 2>/dev/null && echo "GRAPH EXISTS" || echo "NO GRAPH"
```

**If graph exists — run boundary pre-screening:**

Read `gk/graph/DEPENDENCY_GRAPH.json` and check for cross-concept imports
that violate ARCHITECTURE.md patterns:

```python
import json

dep_g = json.load(open('gk/graph/DEPENDENCY_GRAPH.json'))
arch_violations = []
...
Read all `gk/reviews/phase-N-review.md` files. Read `gk/reviews/backlog.md`.
...
Save to `gk/reviews/phase-[N]-review.md`.
...
🔴 Review fixes required — Phase [N] gate (0/N)
   ...
   → Full report: gk/reviews/phase-[N]-review.md
...
## Step 11 — Update gk/reviews/backlog.md

Add all P1/P2/P3 findings. Use the format in `references/REVIEW_REPORT.md`.

---

## Step 12 — Update gk/ARCHITECTURE.md (if gaps found)
...
📋 Report: gk/reviews/phase-[N]-review.md
```
...
- [ ] P1/P2/P3 logged to gk/reviews/backlog.md
- [ ] Gate decision stated clearly
- [ ] No section TBD or skipped
- [ ] Report saved to gk/reviews/phase-[N]-review.md
