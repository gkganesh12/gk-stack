...
name: gk-progress
description: >-
  Displays a live read-only ASCII progress dashboard for the current gk
  project, rendered from gk/TASKS.md and git history. Triggers on
  "/gk-progress", "progress:", "show progress", "how far along are we".
---

# GK Progress Skill

Renders a live ASCII dashboard of the current project's build progress.
Reads gk/TASKS.md and git history. Never modifies any file.
...
1. `gk/TASKS.md` — phases, features, tasks, gates, active work, backlog
2. `git log --oneline -1` — last commit
3. `git log --oneline --since="7 days ago" | wc -l` — commits this week
4. `gk/bugs/` — count folders, note any active
5. `gk/backlog/` — count files (deferred items)
6. `gk/DECISIONS.md` — count D-[ID] entries
7. `gk/cost/history.json` — total project cost if exists (read silently, skip if missing)
8. `gk/.doctor-last-run` — timestamp of last doctor: run (skip if missing)
...
**Backlog:**
Count items in gk/backlog/ folder.
List names if ≤3 items; show count only if >3.

**Cost (if gk/cost/history.json exists):**
```bash
node -e "
const h = require('./gk/cost/history.json');
...
**Env health (if gk/.doctor-last-run exists):**
```bash
if [ -f "gk/.doctor-last-run" ]; then
  last=$(cat gk/.doctor-last-run)
  now=$(date +%s)
  age=$(( (now - last) / 3600 ))
