...
description: >-
  ...
  complete gk/ folder. Use whenever the user is starting a greenfield
...
Produces CLAUDE.md at project root and the full gk/ folder the AI coding agent uses.
...
- `gk/TASKS.md` — your single progress view. Plain English. Updated after every task.
...
- `CLAUDE.md` · `gk/ARCHITECTURE.md` · `gk/CODEBASE.md`
- `gk/SPEC.md` · `gk/SPEC_INDEX.md` · `gk/PLAN.md` · `gk/DECISIONS.md`
- `gk/DESIGN_SYSTEM.md` *(created by design: — agent reads for design context)*
- `gk/reviews/` · `gk/features/` · `gk/bugs/` · `gk/backlog/`
...
Note: `gk/DESIGN_SYSTEM.md` is not created here — it is generated the first time
...
Wait for approval. Save as `gk/SPEC.md`.
...
Present wireframe. Iterate until approved. Append UI Specification to gk/SPEC.md:
...
## Step 4 — Generate gk/SPEC_INDEX.md

Read gk/SPEC.md in full. Generate SPEC_INDEX.md — compressed map under 40 lines:
...
## Backlog — [N items deferred] → gk/backlog/
```

Save as `gk/SPEC_INDEX.md`.
Rule: SPEC.md and SPEC_INDEX.md always committed together. Never one without the other.

---

## Step 5 — Generate gk/PLAN.md

Read `references/PLAN_MD.md` for the full template.
Read gk/SPEC.md in full. Read ARCHITECTURE.md if it exists.
...
Read gk/SPEC.md features section. Read BRIEF.md feature list and non-goals.
...
**Step 5D — Save as `gk/PLAN.md`.**

The feature map in PLAN.md is the source of truth for:
- TASKS.md Phase 2+ section (generated from it in Step 7)
- gk-add-feature (reads it before drafting each feature spec)
- gk-change-spec (reads it to assess phase impact of scope changes)
- gk-parallel (reads dependency diagram for wave planning)

---

## Step 6 — Generate CLAUDE.md (project root)

Read gk/SPEC.md, gk/PLAN.md, gk/ARCHITECTURE.md (if exists).
...
5. Commit doc updates separately:
   git add gk/TASKS.md gk/DECISIONS.md gk/CODEBASE.md
   git commit -m "docs(TASKS): mark [TASK-ID] done — [plain English]"
6. Update "What just happened" and "What's next" in gk/TASKS.md
7. Re-read gk/TASKS.md silently
8. State next task in plain English and confirm before starting

Rules:
- NEVER skip the commit step — uncommitted work is invisible to gk-graph and gk-review
...
## Step 7 — Generate gk/TASKS.md

Read `references/TASKS_MD.md` for full template and update rules.
Read gk/SPEC.md, gk/SPEC_INDEX.md, gk/PLAN.md — especially Section 6 (Feature Map).
...
Start Phase 1: read CLAUDE.md then gk/TASKS.md. Say "next" for each task.
...
## Step 8 — Generate gk/CODEBASE.md (placeholder)
...
Generate the placeholder version. Save as `gk/CODEBASE.md`.

---

## Step 9 — Generate remaining gk/ files and handle ARCHITECTURE.md
...
**If ARCHITECTURE.md exists at project root:** Move to `gk/ARCHITECTURE.md`.

**If ARCHITECTURE.md does not exist** (architect: was skipped):
Generate a minimal `gk/ARCHITECTURE.md` from PLAN.md decisions:
...
> "gk/ folder created. Running spec-review before first task..."

Invoke `gk-spec-review` with:
...
>
> **One file to watch:** `gk/TASKS.md`
>
> **Structure created:**
> ```
> CLAUDE.md
> gk/TASKS.md        ← open this
> gk/ARCHITECTURE.md ← [locked from architect: / auto-generated from plan]
> gk/CODEBASE.md     ← placeholder until Phase 1 done
> gk/SPEC.md + SPEC_INDEX.md + PLAN.md + DECISIONS.md
> gk/reviews/backlog.md
> gk/spec-reviews/   ← spec review history
> gk/graph/          ← dependency graph (spec graph initialised)
> ```
...
> ```
>
> If `autonomous` → read `~/.claude/skills/gk-mode/references/AUTONOMOUS_EXECUTION_BLOCK.md`
> and follow the autonomous execution protocol for Phase 1 tasks. Begin immediately.
>
> If `manual` or not set → tell the user:
> ```
> Read CLAUDE.md, then gk/CODEBASE.md, then gk/ARCHITECTURE.md,
> then gk/SPEC_INDEX.md, then gk/TASKS.md.
