...
The canonical PLAN.md template for gk-* projects.
Used by gk-new-app to generate PLAN.md.
...
Downstream consumers and what they read:
  gk-new-app:     generates TASKS.md from the feature map
  gk-add-feature: reads feature map before drafting spec — checks dependencies
  gk-change-spec: reads feature map to assess phase impact of scope changes
  gk-review:      reads phase boundaries to understand scope of each gate
...
**Spec:** `gk/features/[date-slug-feature-1]/` ← created when `feature: [name]` runs
...
**Spec:** `gk/features/[date-slug-feature-2]/` ← created when `feature: [name]` runs
...
**Done when:** [Plain English]
**Spec:** `gk/features/[date-slug-feature-3]/` ← created when `feature: [name]` runs
...
This feeds directly into gk-parallel wave planning.
