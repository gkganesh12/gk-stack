...
Read during Step 4 of gk-cost before generating recommendations.
...
1. **Use gk-context (when available)**
...
For parallel subagents (gk-parallel), this is unavoidable —
...
**What it is:**
gk-parallel spawns N subagents, each with a full context window.
...
**Detection signal:**
- gk-parallel was used this session
...
1. **CLAUDE.md changes frequently**
   ...
   GK_MODE changed), the cache is invalidated every time.
...
**Fix:**
- Install gk-doctor and run at session start
...
| CP-07 | 15-30 min/session | gk-doctor at session start |
