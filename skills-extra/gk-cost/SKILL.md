...
name: gk-cost
description: >-
  Tracks and reports token usage and dollar cost for a gk-* project — per
  ...
  expensive"; also auto-invoked at the end of gk-add-feature, gk-fix-bug,
  gk-new-app, and gk-review.
---
...
Triggered automatically by other gk-* skills at session end.
...
ls gk/cost/ 2>/dev/null && echo "HISTORY EXISTS" || echo "FIRST RUN"

# Read project context
cat CLAUDE.md 2>/dev/null | grep -E "model|MODEL" || echo "model not specified"
cat gk/TASKS.md 2>/dev/null | head -50
```
...
```bash
cat gk/TASKS.md 2>/dev/null
cat gk/DECISIONS.md 2>/dev/null | tail -50  # recent decisions only
ls gk/features/ 2>/dev/null
ls gk/bugs/ 2>/dev/null
cat gk/cost/baseline.json 2>/dev/null       # previous session data
cat gk/cost/history.json 2>/dev/null        # all sessions
```
...
If `gk-parallel` was used this session → multiply total by number of Wave 1 subagents.
...
**Per-feature cost:**
For each feature folder in `gk/features/`, sum tasks attributed to it:
...
Signal: cost spike on sessions using gk-parallel
...
Check: same environment issue debugged in more than one session → flag and point to gk-doctor
...
💡 [CP-01] Context overhead is 48% of this session's cost
   ...
   Fix: Use gk-context to load only the relevant section per task.
...
💡 [CP-05] Cache hit rate: 8% (very low)
   ...
   dynamic sections (GK_MODE, ACTIVE FEATURE). Cache the stable part.
...
# Save as baseline
Path('gk/cost/baseline.json').write_text(json.dumps(session, indent=2))

# Append to history
history_path = Path('gk/cost/history.json')
...
```bash
# gk/cost/report-[YYYY-MM-DD]-[HH-MM].md
```

---

## Step 6b — Write summary.json for gk-ledger

After updating history.json, Claude must write `gk/cost/summary.json` directly.
...
Path('gk/cost/summary.json').write_text(json.dumps(summary, indent=2))
...
Full report: gk/cost/report-[date]-[time].md
History:     gk/cost/history.json ([N] sessions)
```

---

## Step 8 — Update gk docs

**gk/TASKS.md** — append to "What just happened":
```
💰 Cost tracked — Session #[N]: $[X.XX] [est.|actual]
   Project total: $[X.XX] · Trend: [↑/↓/→]
   gk/cost/report-[date].md
```

**gk/DECISIONS.md** — only if a cost recommendation was acted on:
```
---
