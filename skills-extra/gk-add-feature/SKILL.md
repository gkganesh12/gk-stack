...
**Human-facing:**
- `gk/TASKS.md` — single progress view, plain English, updated after every task

**Agent-facing (human never opens these):**
- `gk/features/[date-slug]/FEATURE_SPEC.md`
- `gk/features/[date-slug]/FEATURE_PLAN.md`
- `gk/features/[date-slug]/FEATURE_TASKS.md`
- `gk/CODEBASE.md` · `gk/ARCHITECTURE.md` · `gk/DECISIONS.md` · `CLAUDE.md`
...
Scan `gk/features/` for folders matching the requested feature.
If an incomplete feature folder exists:
> "Found an existing [feature] folder: gk/features/[date-slug]/
> Resume that feature or start fresh with a new folder?"
Wait for answer before proceeding.

**Read gk/CODEBASE.md (or generate it):**

If gk/CODEBASE.md exists:
...
If gk/CODEBASE.md does not exist:
...
Generate gk/CODEBASE.md. Save as `gk/CODEBASE.md`.

**Read gk/ARCHITECTURE.md** if it exists — conventions must be followed throughout.

Confirm:
> "Codebase read. [Generated fresh / Verified existing] CODEBASE.md.
>  [Stack] project. Feature to add: [feature].
>  Proposed folder: gk/features/[date-slug]/
...
- `gk/SPEC.md` — existing features, acceptance criteria, out-of-scope items, data model
- `gk/SPEC_INDEX.md` — for quick orientation to feature relationships
- `gk/PLAN.md` Section 6 (Feature Map) — sequencing, dependencies, shared data
...
Wait for approval. Save as `gk/features/[date-slug]/FEATURE_SPEC.md`.
...
- gk/DESIGN_SYSTEM.md if it exists (existing tokens — must be consistent with these)
...
Read FEATURE_SPEC.md, CODEBASE.md, ARCHITECTURE.md, gk/SPEC.md (integration sections).
...
Present. Wait for approval. Save as `gk/features/[date-slug]/FEATURE_PLAN.md`.
...
> Folder: gk/features/[date-slug]/ | Added: [date]

**Feature summary**: [1-line]
**Files in scope**: [exact list from FEATURE_PLAN.md]
**Files out of scope**: [explicit list]
**Design system**: [Check gk/DESIGN_SYSTEM.md for existing tokens before any styling]

**Conventions** (from gk/CODEBASE.md section 6 + gk/ARCHITECTURE.md):
[Reproduce directly relevant patterns]

**Scope changes**: If user says "change:" — stop and run gk-change-spec immediately.
...
**Session startup:**
1. Read CLAUDE.md · 2. Read gk/CODEBASE.md · 3. Read gk/ARCHITECTURE.md
4. Read gk/DESIGN_SYSTEM.md if exists · 5. Read gk/SPEC_INDEX.md
6. Read gk/TASKS.md · 7. Read FEATURE_TASKS.md
...
4. Stage and commit doc updates separately:
   ```
   git add gk/features/[date-slug]/FEATURE_TASKS.md gk/TASKS.md gk/DECISIONS.md gk/CODEBASE.md
...
Do NOT skip the commit step — uncommitted work is invisible to gk-graph and gk-review.
...
Save as `gk/features/[date-slug]/FEATURE_TASKS.md`.

---

## Step 9 — Update gk/TASKS.md
...
🔄 [Feature Name] — [what this lets users do] (0/N)
   ...
   [ ] [TASK-ID] · [task name] — [one plain English line]
   ...
   → Full specs: gk/features/[date-slug]/FEATURE_TASKS.md (agent use)
```
...
## Step 10 — Update gk/DECISIONS.md

```
---
## — Feature Start: [Feature Name] — [date]
> Folder: gk/features/[date-slug]/
...
```
⬜ [Feature name] → 🔄 [Feature name] — spec in gk/features/[date-slug]/
```
...
- Scope: FEATURE_SPEC.md + gk/SPEC.md (contradiction check)
...
> ✅ **Feature kit ready.**
>
> **Created:** gk/features/[date-slug]/
>   FEATURE_SPEC.md · FEATURE_PLAN.md · FEATURE_TASKS.md
> **Updated:** gk/TASKS.md · CLAUDE.md · gk/CODEBASE.md · gk/DECISIONS.md
...
> ```
> Read CLAUDE.md, gk/CODEBASE.md, gk/ARCHITECTURE.md,
> gk/SPEC_INDEX.md, gk/TASKS.md, then FEATURE_TASKS.md.
...
After the feature is built and tasks are complete, invoke `gk-graph: update`.
