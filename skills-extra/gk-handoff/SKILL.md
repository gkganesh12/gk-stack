...
description: >-
  ...
  a gk-* project. Triggers on "handoff: client", "handoff: milestone",
...
cat gk/SPEC.md 2>/dev/null
cat gk/ARCHITECTURE.md 2>/dev/null
cat gk/CODEBASE.md 2>/dev/null
cat gk/TASKS.md 2>/dev/null
cat gk/DECISIONS.md 2>/dev/null
cat gk/reviews/backlog.md 2>/dev/null
...
```bash
mkdir -p gk/handoff/[mode]-[YYYY-MM-DD]/
```
...
```
gk/handoff/milestone-phase[N]-[date]/
...
Sources: `gk/reviews/backlog.md` + any open bug folders.
...
```bash
python3 ~/.claude/skills/gk-handoff/scripts/generate_portal.py gk/handoff/[mode]-[date]/
```
...
4. Writes `gk/handoff/[mode]-[date]/index.html`
...
> Portal: `gk/handoff/[mode]-[date]/index.html`
>
...
6. Archive: move to gk/handoff/archive/ after sign-off received
```
...
3. Regenerate portal: python3 ~/.claude/skills/gk-handoff/scripts/generate_portal.py gk/handoff/[mode]-[date]/
...
✅ Handoff package generated — [mode] — [date]
   Recipient: [name/role]
   Documents: [N] files + index.html portal in gk/handoff/[mode]-[date]/
