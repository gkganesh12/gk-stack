#!/usr/bin/env python3
"""
gk-ledger generator v1.1
Reads gk/cost/history.json + gk/cost/summary.json
Generates gk/cost/ledger/index.html
...
def find_project_root():
    ...
    for path in [cwd, *cwd.parents]:
        if (path / "gk" / "cost" / "history.json").exists():
            return path
    return None

def load_history(root):
    with open(root / "gk" / "cost" / "history.json") as f:
        return json.load(f)

def load_summary(root):
    p = root / "gk" / "cost" / "summary.json"
    if not p.exists():
        return {}
    try:
...
def render_feature_costs(m):
    ...
    # Normalise — gk-cost may write a list of {label, cost} dicts instead of a plain dict
...
<title>gk-ledger · {project_name}</title>
<style>{CSS}</style>
</head>
<body>
<div class="shell">
...
def main():
    ...
    if not root:
        print("ERROR: gk/cost/history.json not found in current or parent directories.")
        print("Make sure you're inside a gk-* project and have run cost: at least once.")
        sys.exit(1)
