...
name: gk-ledger
description: >-
  Generates a self-contained HTML cost report for the current gk project
  from gk/cost/history.json and opens it in the browser — all plain
...
Reads `gk/cost/history.json`. Writes `gk/cost/ledger/index.html`. Opens in browser.
...
**Automatically** — gk-cost calls this at the end of every session after writing history.json.
...
```bash
ls gk/cost/history.json 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
```bash
python3 ~/.claude/skills/gk-ledger/scripts/generate.py
```

The script:
1. Finds `gk/cost/history.json` from the current project directory
2. Computes all metrics — totals, averages by task size, day breakdowns, efficiency ratios
3. Generates `gk/cost/ledger/index.html` — self-contained, no external dependencies
...
> Report: `gk/cost/ledger/index.html`
>
...
**"gk/cost/history.json not found"**
...
→ Try opening `gk/cost/ledger/index.html` directly in Chrome or Firefox.
