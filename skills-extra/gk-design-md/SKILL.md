...
description: >-
  ...
  from any URL — so gk-design can produce pixel-accurate matching UI.
  ...
  "fetch DESIGN.md for", "create DESIGN.md from"; after output, gk-design
...
gk-design reads this file automatically at session start (in its Step 2)
...
How to use:
  gk-design reads DESIGN.md automatically.
...
If `gk/DESIGN_SYSTEM.md` exists:
> "DESIGN.md is ready. Should I also update gk/DESIGN_SYSTEM.md to use
...
If no — leave as is. DESIGN.md will still be read by gk-design.
...
Not in gk/, not in src/. Root only.
gk-design checks `./DESIGN.md` — nowhere else.
