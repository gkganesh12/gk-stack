...
#
# Read this file when GK_MODE=autonomous is detected.
# Inserted into: gk-add-feature, gk-fix-bug, gk-new-app
...
**Read mode:**
```bash
MODE=$(grep "GK_MODE" CLAUDE.md 2>/dev/null | cut -d= -f2 | tr -d ' ')
```
...
→ Invoke `gk-parallel` (subagent dispatch, no prompt in autonomous mode)
...
→ Automatically invoke `gk-review` for this phase
...
```
Running review: [phase name] automatically (GK_MODE=autonomous)...
```

Invoke `gk-review` for this phase.
...
[gk-new-app: announce Phase 1 done, Phase 2 ready to begin]
[gk-add-feature: announce feature complete, ready for merge]
[gk-fix-bug: announce fix complete, ready for merge]
```
...
⚡ Autonomous mode active — executing all tasks automatically.
   ...
   To switch to manual at any time: gk-mode: manual
```
