...
name: gk-document
description: >-
  Generates complete documentation for gk-* projects — inline code docs,
  ...
  "no documentation"; also runs automatically after gk-init on undocumented
  legacy projects, after gk-add-feature, and when gk-review flags missing
...
### Entry point B — After gk-init
Triggered automatically at the end of a `gk-init` session on a legacy project.
...
### Entry point C — After gk-add-feature
...
### Entry point D — After gk-review flags docs
Triggered when `gk-review` produces P1/P2 findings for missing documentation.
...
1. `gk/ARCHITECTURE.md` — naming conventions, code style, language (affects doc format)
2. `gk/CODEBASE.md` — full file map, routes, components, patterns, stack
3. `gk/SPEC.md` — feature descriptions and acceptance criteria (drives README content)
4. `gk/DECISIONS.md` — all entries (drives CHANGELOG content)
...
Full detail: `gk/ARCHITECTURE.md`
...
*Last updated: [date] · Generated by gk-document*
```
...
| `discovery` | Decisions |
| Bootstrap entry (gk-init) | Project start |
...
Generated from gk/DECISIONS.md — updated by gk-document.
...
[Source: D-[ID] in gk/DECISIONS.md]
...
*Generated by gk-document on [date]*
*Source: gk/DECISIONS.md*
```
...
Read `gk/CODEBASE.md` section 5 (routes/endpoints) in full.
...
Read `gk/CODEBASE.md` section 7 (key components) in full.
...
Mark them resolved: `✅ [finding] — RESOLVED by gk-document [date]`
