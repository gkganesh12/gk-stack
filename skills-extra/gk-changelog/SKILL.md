...
cat gk/features/*/FEATURE_SPEC.md 2>/dev/null
cat gk/bugs/*/BUG_SPEC.md 2>/dev/null
...
**By feature folder:**
Tasks in the same `gk/features/[name]/` folder → one changelog entry.
Use the feature name from FEATURE_SPEC.md, not the folder name or task IDs.

```
gk/features/2026-04-12-web-connect/
...
- GK Match weather integration — 5-day forecast on web discover page,
...
- GK match scoring moved to client-side — reduces server load at current
